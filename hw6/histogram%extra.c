...
int plotHistogram(char * dataFile, float binMin, float binRange, int binAmount, char * percent)
{
	int i, count, numberOfValues, binNumber=1, countValue[1000], percentNum;
	...
	//if % is there then do percentages
	if(percent!="%")
	{
		...
		{	
			...
			for(i=0;i<binAmount;i++)
			{
				...
				newBinMin = binMin + binRange*binNumber; 						//creates new bin min
				...
				percentNum = numberOfValues/(count-2);
				printf("number of values %d the count is %d pecentage is%d",numberOfValues,count,percentNum); //testing
				binNumber++;
				printf("			%d", numberOfValues);
				printStars(percentNum);
				numberOfValues = 0;
				fclose(filePointer);
				count = count - count;
...
int main(int argc, char * argv[])
{
	...
	sscanf(argv[5],"%s",percent);
	

	
	count = plotHistogram(argv[1],binMin,binRange,binAmount,percent);
